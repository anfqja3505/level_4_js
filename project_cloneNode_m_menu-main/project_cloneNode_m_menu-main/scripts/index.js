...
function m_navfunc(color='#222', rightVal='-100%', opacityVal=0){
    m_showHide_Boolean = !m_showHide_Boolean; // !는 반전
    ...
    m_nav_bg.style.transition = `right 0.4s, opacity 1s`
}
...
// m_nav_btn.addEventListener('click',()=>{
    ...
    //사용자가 처음 접속 시 안보이는 걸로 시작하는 지 보이는 걸로 시작하는지에 따라
    //해당 값을 변수의 초기값으로 저장한다.
    //let m_showHide_Boolean = 0;// 안보이는 걸로 시작한 초기값
    //사용자의 행동에 따라 변경된 초기값 예측을 위해 조건식으로 작업 
    //m_showHide_Boolean==0 ? m_navfunc('#fff', '0%', 1) : m_navfunc()
    //조건 ? 참일때 실행 : 거짓일때 실행
    // m_showHide_Boolean 값이 0 참, 거짓일때 각각 다른 결과가 실행
    //참 <-> 거짓이 반복되어야하므로 실행되는 함수 내에 m_showHide_Boolean = !m_showHide_Boolean; 변수 값을 반전시키는 실행값 포함