...
//const q1 = prompt('1+1?');
//alert('사용자 입력한 답은 '+q1+' 입니다') //사용자가 입력한 답은 ? 입니다
//print();
const printBtn = document.querySelector('#print_btn');

//v1. 사용자 정의 함수를 이벤트함수내에서 호출한 방식(2개 이상 반복일 경우)
//printBtn.addEventListener('click',printFunc);

//v2. 이벤트함수 내에서 직접 사용자함수를 만드는 방식(반복안될때)
//이벤트의 객체가 정해진 상태로 하기 때문에 내부 내장함수엔 함수명을 따로 작성안함.
...
const num1 = Math.random();//0~1 랜덤한 숫자 생성 내장 
const num2 = Math.floor(Math.random()*10) + 1;
console.log(num1, num2)

//변수생성 =함수1(함수2()) //함수2부터처리 -> 함수1 처리 -> 변수 대입
...
//동화 : 헨젤과 그레텔이 집이 찾아오는 반복 방법
function story1(boy='헨젤', girl='그레텔', mapData) { //함수생성(매개변수생성)
    let person = [boy, girl];
    let result = person[0]+'과(와) '+person[1]+'이 '+mapData+'을 이용해 집을 찾아갔습니다.'
...
//나무꾼은 금도끼를 잃어렸습니다
//나무꾼은 쇠도끼를 잃어렸습니다
function story2(treePerson='나무꾼', treeExe) {
    let person = treePerson;
    let result = treePerson+'은 '+treeExe+'를 잃어버렸습니다';
    return console.log(result);
}
story2();//매개변수 아주거도 안적었을 때 모두 기본값
story2(undefined, '금도끼');//앞의 매개변수 정의안되서 기본값, 뒤 문자만 매개변수 대입
story2('주인공');//앞 매개변수 문자대입, 뒤 매개변수 안적어서 기본값 처리

//카페 키오스크
//아이스아메리카노 한잔 주문완료
//function 간편버전 => 이벤트 내에서만 사용
// ''+변수+'' 간편버전 `문자${변수}` 템플릿문자열
function cafeKiosk(menu, number=1) {
    const order = `${menu} ${number}잔 주문완료`
    return console.log(order);
}
cafeKiosk('아이스아메리카노', 2);
cafeKiosk('카페라떼', 2);
cafeKiosk('콜드브루', 4);
...
function cafeOrder(tem, me, num=1) {
    const temperature = ['ICE', 'HOT'];
    const menu = ['아메리카노', '카페라떼', '녹차라떼', '초코라떼'];
    const order = `${temperature[tem]} ${menu[me]} ${num}잔 주문 완료`;
    return console.log(order);
}
cafeOrder(1, 1, 1);
...
function japangi(drink, num=1) {
    ...
    return order;//order 변수에 저장된 값만 함수 외부로 반환(종료)
    return;//리턴 뒤에 아무것도 안적으면 함수만 강제 종료
    return console.log('잔액이 부족합니다.')//리턴 뒤 콘솔 실행하고 함수 종료
    console.log(order)//리턴 아래 써있는 명령은 절대 실행 x
}
japangi('생수'); //함수 내부 리턴에서 출력함수가 정해졌을 때 테이터만 전달방식
